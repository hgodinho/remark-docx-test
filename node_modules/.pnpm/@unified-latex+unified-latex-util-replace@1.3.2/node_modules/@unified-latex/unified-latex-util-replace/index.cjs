...
function replaceStreamingCommand(ast, isStreamingCommand, replacer, options) {
  ...
  if (Array.isArray(ast)) {
    ...
    const envThatDoesntBreak = import_unified_latex_util_match4.match.createEnvironmentMatcher(
      environmentsThatDontBreakPars
    );
    ...
    splitByPar.separators = splitByPar.separators.map(
      (sep) => import_unified_latex_util_match4.match.macro(sep, "par") ? { type: "parbreak" } : sep
    );
    const replacers = [];
