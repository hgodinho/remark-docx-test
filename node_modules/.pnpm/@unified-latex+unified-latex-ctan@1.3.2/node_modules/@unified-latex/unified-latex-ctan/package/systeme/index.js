...
function normalizeVariableWhitelist(vars) {
  ...
  const ret = normalized.filter(
    (v) => (match2.anyMacro(v) || match2.anyString(v)) && !match2.string(v, " ") && !match2.whitespace(v)
  );
  return ret;
}
  ...
  visit(
    ...
    (nodes, info) => {
      ...
      const systemeLocations = nodes.flatMap(
        (node, i) => match2.macro(node, "systeme") ? i : []
      );
      const sysdelimLocations = nodes.flatMap(
        (node, i) => match2.macro(node, "sysdelim") ? i : []
      );
      ...
      for (const i of systemeLocations) {
        ...
        if (lastSysdelim >= 0) {
          ...
          if (!match2.anyMacro(sysdelimMacro)) {
            throw new Error(
              `Expecting sysdelim macro but found "${printRaw2(
                sysdelimMacro
              )}"`
            );
          }
