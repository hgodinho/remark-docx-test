...
var tikzCommandArgumentParser = (nodes, startPos) => {
  ...
  while (match4.whitespace(nodes[pos])) {
    pos++;
  }
  ...
  if (match4.group(firstNode)) {
    const args = [animationArg, optionalArg, arg2(firstNode.content)];
  ...
  if (semicolonPosition != null) {
    ...
    trim3(argNodes);
    const args = [animationArg, optionalArg, arg2(argNodes)];
    return {
      args,
...
function eatAllAnimationSpecs(nodes, startPos) {
  ...
  for (; i < nodes.length; i++) {
    const node = nodes[i];
    if (match4.string(node, "[")) {
      break;
    }
    if (match4.string(node, "=")) {
      i++;
      while (match4.whitespace(nodes[i]) || match4.comment(nodes[i])) {
        i++;
      }
      if (!match4.group(nodes[i])) {
        break;
      }
      lastMatchPos = i + 1;
...
function createMatchers() {
  return {
    isChar: match5.string,
    isTerminal: (node) => match5.string(node, ";"),
    isOperation: (node) => match5.anyString(node) && node.content.match(/[a-zA-Z]/),
    isWhitespace: (node) => match5.whitespace(node) || match5.parbreak(node),
    isComment: match5.comment,
    isGroup: match5.group,
    isMacro: match5.macro,
    isAnyMacro: match5.anyMacro
  };
}
