...
var import_unified_latex_util_replace = require("@unified-latex/unified-latex-util-replace");
...
function cleanEnumerateBody(ast, itemName = "item") {
  let { segments, macros: macros15 } = (0, import_unified_latex_util_split.splitOnMacro)(ast, itemName);
  for (let i = 0; i < segments.length; i++) {
    const segment = segments[i];
    if (i === 0) {
      (0, import_unified_latex_util_trim.trimEnd)(segment);
    } else {
      (0, import_unified_latex_util_trim.trim)(segment);
    }
  ...
  for (let i = 0; i < body.length - 1; i++) {
    ...
    if (!import_unified_latex_util_match.match.parbreak(nextNode)) {
      continue;
    }
    if (import_unified_latex_util_match.match.comment(node)) {
      node.suffixParbreak = true;
    }
    if (import_unified_latex_util_match.match.macro(node) && node.args && node.args[node.args.length - 1].closeMark === "") {
      const args = node.args[node.args.length - 1].content;
      const lastArg = args[args.length - 1];
      if (import_unified_latex_util_match.match.comment(lastArg)) {
...
function popTrailingComments(nodes) {
  let lastNodeIndex = (0, import_unified_latex_util_replace.lastSignificantNodeIndex)(nodes, true);
...
var environments5 = {
  document: {
    processContent: (nodes) => {
      (0, import_unified_latex_util_trim2.trim)(nodes);
      return nodes;
    }
  },
...
function blankArg() {
  return (0, import_unified_latex_builder2.arg)([], { openMark: "", closeMark: "" });
}
var tikzCommandArgumentParser = (nodes, startPos) => {
  ...
  if (cursorPosAfterAnimations !== pos) {
    ...
    (0, import_unified_latex_util_trim3.trim)(argContent);
    animationArg = (0, import_unified_latex_builder2.arg)(argContent, {
      openMark: " ",
      closeMark: " "
    });
  }
  ...
  } = (0, import_unified_latex_util_arguments2.gobbleSingleArgument)(nodes, OPTIONAL_ARGUMENT_ARG_SPEC, pos);
  nodesRemoved += optionalArgumentNodesRemoved;
  const optionalArg = _optionalArgument || blankArg();
  while (import_unified_latex_util_match4.match.whitespace(nodes[pos])) {
    pos++;
  }
  ...
  if (import_unified_latex_util_match4.match.group(firstNode)) {
    const args = [animationArg, optionalArg, (0, import_unified_latex_builder2.arg)(firstNode.content)];
  ...
  if (semicolonPosition != null) {
    ...
    (0, import_unified_latex_util_trim3.trim)(argNodes);
    const args = [animationArg, optionalArg, (0, import_unified_latex_builder2.arg)(argNodes)];
    return {
      args,
...
function eatAllAnimationSpecs(nodes, startPos) {
  ...
  for (; i < nodes.length; i++) {
    const node = nodes[i];
    if (import_unified_latex_util_match4.match.string(node, "[")) {
      break;
    }
    if (import_unified_latex_util_match4.match.string(node, "=")) {
      i++;
      while (import_unified_latex_util_match4.match.whitespace(nodes[i]) || import_unified_latex_util_match4.match.comment(nodes[i])) {
        i++;
      }
      if (!import_unified_latex_util_match4.match.group(nodes[i])) {
        break;
      }
      lastMatchPos = i + 1;
...
function processTikzEnvironmentContent(nodes) {
  (0, import_unified_latex_util_arguments3.attachMacroArgsInArray)(nodes, conditionalMacros);
  return nodes;
}
...
var import_unified_latex_util_pegjs4 = require("@unified-latex/unified-latex-util-pegjs");
function createMatchers() {
  return {
    isChar: import_unified_latex_util_match5.match.string,
    isTerminal: (node) => import_unified_latex_util_match5.match.string(node, ";"),
    isOperation: (node) => import_unified_latex_util_match5.match.anyString(node) && node.content.match(/[a-zA-Z]/),
    isWhitespace: (node) => import_unified_latex_util_match5.match.whitespace(node) || import_unified_latex_util_match5.match.parbreak(node),
    isComment: import_unified_latex_util_match5.match.comment,
    isGroup: import_unified_latex_util_match5.match.group,
    isMacro: import_unified_latex_util_match5.match.macro,
    isAnyMacro: import_unified_latex_util_match5.match.anyMacro
  };
}
...
var import_unified_latex_util_pegjs5 = require("@unified-latex/unified-latex-util-pegjs");
...
var import_unified_latex_util_arguments4 = require("@unified-latex/unified-latex-util-arguments");
...
var import_unified_latex_builder3 = require("@unified-latex/unified-latex-builder");
